...
const ListAdmins = () => {
  ...
  // const admins = useSelector((state) => state.users.data.find(user => user.userRoleId == "admin"));
  const admins = useSelector((state) =>
    state.users.data.filter((user) => user.userRoleId == 2)
  );
  ...
  const handleCreateAdminAction = async (e) => {
    await dispatch(createAdminAction(userData)).catch((e) => console.error(e));
    setOpen(false);
  };
